...
if __name__ == '__main__':
    ...
    image_tree_root = json.loads(open(sys.argv[2], 'r').read())
    output = {}
    reverse_output = {}
    for hotelid in image_tree_root:
        output[hotelid] = []
        reverse_output[hotelid] = {}
        image_tree = image_tree_root[hotelid]
        for index in image_tree:
            ...
            for res in tree['results']:
                ...
                element['url'] = url
                element['path'] = path
                output[hotelid].append(element)
                str_path = str(map(lambda x: x[0], path))
                if str_path not in reverse_output[hotelid]:
                    reverse_output[hotelid][str_path] = []
                reverse_output[hotelid][str_path].append(url)
